...
static char *GetWindowTitle(void)
{
   ...
   if (filename)
   {
      ...
      window_title = strdup("MPlayer " AMIGA_VERSION " (sdl_sm502)");
   }
...
//	"SDL YUV/RGB/BGR renderer (SDL v1.1.7+ only!)",
	"SDL Alpha version for QEMU SM502 fix G4 problem (NOHWSURFACE)",
	"sdl_sm502",
    ...
    */
	...
	if (priv->mode == YUV && priv->bpp < 16) {
 		    ...
 		    mp_msg(MSGT_VO,MSGL_V, "SDL: Your SDL display target wants to be at a color "
                           ...
                           priv->bpp);
//		printf ("!!fdsfsdfsdfsdfsfssf \n");

//markus rem 		priv->bpp = 16;
	}
...
static int preinit(const char *arg)
{
    ...
    /* default to no fullscreen mode, we'll set this as soon we have the availible modes */
    priv->fullmode = -2;
  ...
  /* markus simple test to see if it's a QEMU emulated Pegasos2 SM502
     I don't think anyone with a real Amiga will uses 16pp.
     UAE use it, but the CPU does not emulate the AltiVec
  */
     supported_bpp=SDL_VideoModeOK(640, 480, 32, SDL_FULLSCREEN);
                if (!SDL_HasAltiVec() && (supported_bpp > 16))
		{
		    mp_msg(MSGT_VO, MSGL_INFO, "VO: [sdl_sm502] Sorry, I don't need to use -vo sdl\n");
		    return -1;
		}
