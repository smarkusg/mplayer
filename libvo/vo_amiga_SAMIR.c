...
{
    "Picasso96 AmigaOS video output",
    "amiga",
    "Jrg Strohmayer",
...
int amiga_pip_in_use()
{
	char ret[100];
	sprintf(ret,"0");

	GetVar("amiga_pip_in_use", ret , 100, GVF_GLOBAL_ONLY );
	if (ret[0] == '1')
	{
		return 1;
	}
...
static int preinit(const char *arg)
{
	...
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [amiga] Config is wrong\n");
		return -1;
	}
	...
	if ( ( My_Screen = LockPubScreen ( gfx_screenname ) ) )
	{
		...
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [amiga] Screen use driver: %s\n",ChipDriverName);

		UnlockPubScreen(NULL, My_Screen);

	if (have_radeonhd == TRUE)
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [amiga] Does not support RadeonHD, RadeonRX and SM502 \n");
		return -1;
	}

	// Only one MPlayer window can be opened with vo_amiga

	if (amiga_pip_in_use())
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [amiga] Only one Amiga PIP window can be opened\n");
		return -1;
	}
         ...
         if (menu)
         {
            if (IGadTools->LayoutMenus(menu, vi, GTMN_NewLookMenus, TRUE, TAG_DONE))
            {
               ...
               if ((dri = IIntuition->GetScreenDrawInfo(My_Screen)))
               {
                  ...
                  if (0 == pip_format)
                  {
                     static struct Hook backfillhook = { {NULL, NULL}, (HOOKFUNC)BackFillfunc, NULL, NULL };
...
//                        WA_SmartRefresh,    TRUE,
                        ...
                        WA_IDCMP,           ALWAYSBORDER == BorderMode ? /*IDCMP_SIZEVERIFY | */IDCMP_CHANGEWINDOW | IDCMP_RAWKEY | IDCMP_VANILLAKEY | IDCMP_CLOSEWINDOW | IDCMP_MENUPICK
                                                                       : IDCMP_RAWKEY | IDCMP_VANILLAKEY | IDCMP_CLOSEWINDOW | IDCMP_MENUPICK,
...
//   vo_draw_alpha_func = vo_get_draw_alpha(format);
   ...
   out_format = 0 == pip_format ? IMGFMT_RGB16 : IMGFMT_YV12;
	...
	if (My_Window)
	{
		SetVar("amiga_pip_in_use","1",1,GVF_GLOBAL_ONLY);
		attach_menu(My_Window);
		rp = My_Window->RPort;
	}
	else
	{
		SetVar("amiga_pip_in_use","0",1,GVF_GLOBAL_ONLY);
		return 1;
	}
