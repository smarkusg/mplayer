...
static char *GetWindowTitle(void)
{
   ...
   if (filename)
   {
      ...
      window_title = strdup("MPlayer " AMIGA_VERSION " (comp_yuv)");
   }
...
{
	"Composition, yuv420p, direct rendering",
	"comp_yuv",
	"Kjetil Hvalstrand",
...
static int preinit(const char *arg)
{
	...
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] You need graphics.library version 54.100 or newer\n");	
		return -1;
	}

	if ( ! LIB_IS_AT_LEAST(LayersBase, 54,2) )
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] You need layers.library version 54.2 or newer\n");	
		return -1;
	}
...
//	mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] Welcome man !\n");
	...
	if (!gfx_GiveArg(arg))
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] Config is wrong\n");	
		return -1;	
	}
	...
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] You need a Radeon HD/RX gfx card to use this video output\n");
		return -1;
	}

	if (have_compositing == FALSE)
	{
		mp_msg(MSGT_VO, MSGL_INFO, "VO: [comp_yuv] Screen mode does not have compositing\n");	
		return -1;
	}
...
//					WA_StayTop,         (is_ontop==1) ? TRUE : FALSE,
			...
			default:
					...
					// #endif
					// #ifdef __amigaos4__
					// WA_Title,	"MPlayer " VERSION " (comp_yuv)",
					// #endif
					WA_ScreenTitle,	AMIGA_VERSION " (comp_yuv)",
...
if(is_fullscreen)
{
	...
	My_Window = IIntuition->OpenWindowTags( NULL,
			...
			WA_Activate,        TRUE,
			WA_NewLookMenus,    TRUE,
			WA_IDCMP,           IDCMP_MOUSEMOVE | IDCMP_MENUPICK | IDCMP_MENUVERIFY | IDCMP_MOUSEBUTTONS | IDCMP_RAWKEY | IDCMP_VANILLAKEY | IDCMP_EXTENDEDMOUSE | IDCMP_REFRESHWINDOW | IDCMP_INACTIVEWINDOW | IDCMP_ACTIVEWINDOW,
			WA_Flags,           WFLG_REPORTMOUSE,
		TAG_DONE);
	...
	if (mpi->flags & (MP_IMGFLAG_ACCEPT_STRIDE | MP_IMGFLAG_ACCEPT_ALIGNED_STRIDE | MP_IMGFLAG_ACCEPT_WIDTH))
	{
		if (! mpi->planes[0] )
		{
			...
			if (mpi->flags & MP_IMGFLAG_PLANAR)
			{
				...
				{
					// I420
					mpi->planes[1] = (uint32) yuvInfo.VMemory;
					mpi->planes[2] = (uint32) yuvInfo.UMemory;
				...
				} else
				{
					...
					mpi->planes[1] = (uint32) yuvInfo.UMemory;
					mpi->planes[2] = (uint32) yuvInfo.VMemory;
